...
var log = (0, debug_1.debug)('defineRelationalProperties');
function defineRelationalProperties(entity, initialValues, relations, dictionary, db) {
    ...
    try {
        for (var relations_1 = __values(relations), relations_1_1 = relations_1.next(); !relations_1_1.done; relations_1_1 = relations_1.next()) {
            ...
            (0, outvariant_1.invariant)(dictionary[relation.target.modelName], 'Failed to define a "%s" relational property to "%s" on "%s": cannot find a model by the name "%s".', relation.kind, propertyPath.join('.'), entity[glossary_1.ENTITY_TYPE], relation.target.modelName);
            var references = (0, get_1["default"])(initialValues, propertyPath);
            ...
            log("setting relational property \"".concat(entity[glossary_1.ENTITY_TYPE], ".").concat(propertyPath.join('.'), "\" with references: %j"), references, relation);
