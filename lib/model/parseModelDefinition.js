...
var log = (0, debug_1.debug)('parseModelDefinition');
/**
    ...
    try {
        for (var _b = __values(Object.entries(definition)), _c = _b.next(); !_c.done; _c = _b.next()) {
            ...
            var propertyPath = parentPath
                ? __spreadArray(__spreadArray([], __read(parentPath), false), [propertyName], false) : [propertyName];
            // Primary key.
            if (value instanceof primaryKey_1.PrimaryKey) {
                (0, outvariant_1.invariant)(!result.primaryKey, 'Failed to parse a model definition for "%s": cannot have both properties "%s" and "%s" as a primary key.', modelName, result.primaryKey, propertyName);
                (0, outvariant_1.invariant)(!parentPath, 'Failed to parse a model definition for "%s" property of "%s": cannot have a primary key in a nested object.', parentPath === null || parentPath === void 0 ? void 0 : parentPath.join('.'), modelName);
...
function parseModelDefinition(dictionary, modelName, definition) {
    ...
    (0, outvariant_1.invariant)(result.primaryKey, 'Failed to parse a model definition for "%s": model is missing a primary key. Did you forget to mark one of its properties using the "primaryKey" function?', modelName);
    return result;
}
