...
var log = (0, debug_1.debug)('relation');
var RelationKind;
...
var Relation = /** @class */ (function () {
    ...
    Relation.prototype.apply = function (entity, propertyPath, dictionary, db) {
        ...
        var targetPrimaryKey = (0, findPrimaryKey_1.findPrimaryKey)(this.dictionary[this.target.modelName]);
        (0, outvariant_1.invariant)(targetPrimaryKey, 'Failed to create a "%s" relation to "%s": referenced model does not exist or has no primary key.', this.kind, this.target.modelName);
    ...
    Relation.prototype.resolveWith = function (entity, refs) {
        ...
        var exception = function (predicate, reason) {
            ...
            outvariant_1.invariant.apply(void 0, __spreadArray([predicate, "Failed to resolve a \"%s\" relationship to \"%s\" at \"%s.%s\" (%s: \"%s\"): ".concat(reason), _this.kind,
                _this.target.modelName,
                _this.source.modelName,
                _this.source.propertyPath,
                _this.source.primaryKey,
                entity[_this.source.primaryKey]], __read(positionals), false));
        };
        (0, outvariant_1.invariant)(this.source, 'Failed to resolve a "%s" relational property to "%s": relation has not been applied (source: %s).', this.kind, this.target.modelName, this.source);
        ...
        if (this.attributes.unique) {
            ...
            var extraneousEntities = (0, executeQuery_1.executeQuery)(this.source.modelName, this.source.primaryKey, {
                where: (0, set_1["default"])((_a = {},
            ...
            if (extraneousEntities.length > 0) {
                var extraneousReferences_1 = extraneousEntities.flatMap(function (extraneous) {
                    var references = [].concat((0, get_1["default"])(extraneous, _this.source.propertyPath));
        ...
        this.setValueResolver(entity, function () {
            var queryResult = referencesList.reduce(function (result, ref) {
                var _a;
                return result.concat((0, executeQuery_1.executeQuery)(_this.target.modelName, _this.target.primaryKey, {
                    where: (_a = {},
            ...
            return _this.kind === RelationKind.OneOf ? (0, first_1.first)(queryResult) : queryResult;
        });
    };
    Relation.prototype.setValueResolver = function (entity, resolver) {
        ...
        (0, outvariant_1.invariant)(entity[glossary_1.ENTITY_TYPE], 'Failed to set a value resolver on a "%s" relationship to "%s" at "%s.%s": provided object (%j) is not an entity.', this.kind, this.target.modelName, this.source.modelName, this.source.propertyPath.join('.'), entity);
        (0, definePropertyAtPath_1.definePropertyAtPath)(entity, this.source.propertyPath, {
