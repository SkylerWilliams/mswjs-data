...
export type WeakQuerySelector<EntityType extends AnyObject> = Partial<QuerySelector<EntityType>>;
export type RecursiveQuerySelectorWhere<Value extends ModelValueType> = Value extends Array<infer ItemType> ? Partial<GetQueryFor<ItemType>> : Value extends ModelValueType ? Partial<GetQueryFor<Value>> : Value extends AnyObject ? {
...
export type RecursiveOrderBy<Value extends ModelDefinitionValue> = Value extends ModelValueType ? SortDirection : Value extends AnyObject ? DeepRequiredExactlyOne<{
...
export type BulkQueryOptions<EntityType extends AnyObject> = BulkQueryOffsetOptions<EntityType> | BulkQueryCursorOptions<EntityType>;
export type ComparatorFn<ExpectedType extends any, ActualType extends any> = (expected: ExpectedType, actual: ActualType) => boolean;
export type QueryToComparator<QueryType extends StringQuery | NumberQuery | BooleanQuery | DateQuery> = {
    [Key in keyof QueryType]: ComparatorFn<QueryType[Key], QueryType[Key] extends Array<infer ValueType> ? ValueType : QueryType[Key]>;
};
export type GetQueryFor<ValueType extends any> = ValueType extends string ? StringQuery : ValueType extends number ? NumberQuery : ValueType extends Boolean ? BooleanQuery : ValueType extends Date ? DateQuery : ValueType extends Array<infer ItemType extends AnyObject> ? QuerySelector<ItemType>['where'] : ValueType extends Value<any, any> ? QuerySelector<ValueType>['where'] : never;
